  ...
  <div>
    ...
    <table class="table table-sm mr-small table-hover table-bordered col-md-12">
      ...
      <tr class="mr-auto-size">
        <td v-for="head in table_header" v-on:click="mr_click_head_field(head)"
            v-bind:class="typeof head['sort'] !== 'undefined' ? 'mr_cursor' : ''">
          <div v-if="with_checkbox && head['name'] === '#checkbox'">
...
export default {
  ...
  mounted() {
    ...
    if (this.mr_object.body) {
      this.table_body = this.mr_object.body;
      this.table_header = this.mr_object.header;

      this.count_rows = this.mr_object.count;
      this.btn_selected = this.mr_object.btn_selected;
      this.token = this.mr_wait = false;
      this.load_data = false;
      this.message = this.mr_object.result;

      if (this.mr_object['is_checkboxes']) {
        this.with_checkbox = this.mr_object['is_checkboxes'];
      }
    }
  ...
  methods: {
    ...
    SendRequest: function (data) {
      ...
      axios.post(this.mr_route + this.url_param, data).then(response => {
            ...
            if (response.data['is_checkboxes']) {
              this.with_checkbox = response.data['is_checkboxes'];
            }
          }
      );
    },
    ...
    mr_click_head_field(head_name) {
      if (typeof (head_name['sort']) != "undefined" && head_name['sort'] !== null) {
        ...
        // Если нет роута - сортировка в пределах имеющегося списка
        if (this.mr_route) {
          this.List();
        }
      }
